...
export const PAGINAS_CON_RESENAS = [
  {
    ...
    clasesExtra: '',
    etiqueta: 'Opiniones en Google',
    titulo: 'Lo que escribieron en Google',
    bajada:
      'Opiniones publicadas por personas que hicieron terapia en la escuela. Están tal cual aparecen en nuestra ficha de Google Maps.',
  },
  {
    ...
    // Mismo verde agua suave que ya usan las otras franjas de esta página.
    clasesExtra: 'greviews--tint',
    etiqueta: 'Opiniones en Google',
    titulo: 'Lo que escribieron en Google',
    bajada:
      'Están copiadas de nuestra ficha de Google Maps, con el nombre de quien las escribió. Se corrigieron espacios y tildes, nada más.',
  },
];
...
export function generarSeccion(datos, pagina) {
  ...
  let lista = pagina.soloDestacadas ? todas.filter((o) => o.destacada) : todas.slice();
  ...
  lista.forEach((op) => partes.push(tarjeta(op, urlOpiniones)));
  partes.push('</div>');
