...
export const MARCA_COMPARTIR_INICIO = '<!-- COMPARTIR:inicio -->';
export const MARCA_COMPARTIR_FIN = '<!-- COMPARTIR:fin -->';
...
export const PAGINAS_FIJAS = [
  ...
  { loc: 'https://escuelaconconfianza.com/servicios/sesiones-individuales/', lastmod: '2026-07-05', priority: '0.8' },
  ...
  // Fichas de una cara, publicadas el 29/08/2026.
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/', lastmod: '2026-08-29', priority: '0.8' },
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/tartamudez-en-inicial/', lastmod: '2026-08-29', priority: '0.8' },
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/tartamudez-en-primaria/', lastmod: '2026-08-29', priority: '0.8' },
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/tartamudez-en-secundaria/', lastmod: '2026-08-29', priority: '0.8' },
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/tartamudez-en-la-universidad/', lastmod: '2026-08-29', priority: '0.8' },
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/tartamudez-cuando-se-traba-al-hablar/', lastmod: '2026-08-29', priority: '0.8' },
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/hablar-de-la-tartamudez-en-casa/', lastmod: '2026-08-29', priority: '0.8' },
  { loc: 'https://escuelaconconfianza.com/recursos-didacticos/fichas-de-tartamudez/tartamudez-hablar-con-tu-profesor/', lastmod: '2026-08-29', priority: '0.8' },
  // Sección de Anuncios y el evento del Día de la Tartamudez (22/09/2026).
  { loc: 'https://escuelaconconfianza.com/anuncios/', lastmod: '2026-09-22', priority: '0.7' },
  { loc: 'https://escuelaconconfianza.com/anuncios/tartamudez-en-la-plaza/', lastmod: '2026-09-22', priority: '0.8' },
  // Página legal. Va en el sitemap (a diferencia del Libro de Reclamaciones, que
  // es noindex) porque Meta exige una URL de política de privacidad pública y
  // alcanzable para poder publicar la app del bot de WhatsApp.
  { loc: 'https://escuelaconconfianza.com/privacidad/', lastmod: '2026-07-31', priority: '0.3' },
];
...
// Botones para compartir la lectura. Van al final del artículo, antes del
// "Sigue leyendo". Sin librerías: son enlaces normales, y el de copiar lo
// atiende script.js leyendo el atributo data-copiar.
//
// WhatsApp va por api.whatsapp.com (no por wa.me) porque es el dominio oficial
// para compartir texto y funciona igual en celular y en computadora.
export function generarCompartir(articulo) {
  const url = urlAbsoluta(articulo.url);
  const titulo = String(articulo.title || '');

  const paraWhatsapp = encodeURIComponent(titulo + ' ' + url);
  const paraFacebook = encodeURIComponent(url);

  return (
    '\n  <section class="container article-share" aria-labelledby="compartir-title">' +
    '\n    <div class="article-share__inner">' +
    '\n      <p class="article-share__title" id="compartir-title">Si crees que le puede servir a alguien, compártelo</p>' +
    '\n      <div class="article-share__buttons">' +
    '\n        <a class="article-share__btn article-share__btn--wa" href="https://api.whatsapp.com/send?text=' + paraWhatsapp + '" target="_blank" rel="noopener" aria-label="Compartir esta lectura por WhatsApp">' +
    '<i class="fa-brands fa-whatsapp" aria-hidden="true"></i><span>WhatsApp</span></a>' +
    '\n        <a class="article-share__btn article-share__btn--fb" href="https://www.facebook.com/sharer/sharer.php?u=' + paraFacebook + '" target="_blank" rel="noopener" aria-label="Compartir esta lectura en Facebook">' +
    '<i class="fa-brands fa-facebook-f" aria-hidden="true"></i><span>Facebook</span></a>' +
    '\n        <button type="button" class="article-share__btn article-share__btn--link" data-copiar="' + escapeHtml(url) + '" aria-label="Copiar el enlace de esta lectura">' +
    '<i class="fa-solid fa-link" aria-hidden="true"></i><span>Copiar enlace</span></button>' +
    '\n      </div>' +
    '\n    </div>' +
    '\n  </section>\n  '
  );
...
export function actualizarPaginaArticulo(html, articulo, articulos) {
  ...
  // El bloque de compartir va JUSTO DESPUÉS de </article>, no antes de una
  // etiqueta, así que necesita su propia colocación.
  function ponerCompartir(texto, contenido) {
    const inicio = texto.indexOf(MARCA_COMPARTIR_INICIO);
    const fin = texto.indexOf(MARCA_COMPARTIR_FIN);

    if (inicio !== -1 && fin !== -1 && fin > inicio) {
      return texto.slice(0, inicio + MARCA_COMPARTIR_INICIO.length) + contenido + texto.slice(fin);
    }

    const cierre = texto.indexOf('</article>');
    if (cierre === -1) return null;
    const corte = cierre + '</article>'.length;
    return texto.slice(0, corte) + '\n  ' + MARCA_COMPARTIR_INICIO + contenido + MARCA_COMPARTIR_FIN + texto.slice(corte);
  }
  ...
  const conCompartir = ponerCompartir(resultado, generarCompartir(articulo));
  if (conCompartir === null) return { error: 'No se encontró </article> en la página del artículo.' };
  resultado = conCompartir;
